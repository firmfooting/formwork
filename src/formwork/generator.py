...
#
# Transport facts ported from dbml-sharepoint v0.4.0 (read 2026-09-06), each
# cited again at its point of use in the prelude. They are knowledge, not a
# shared runtime: the site prefix from location.pathname, the absence of a
# baked SITE_URL, the real-etag MERGE and the refusal to run off a
# /SitePages/ page are formwork's own measured differences and stay.
# - a throttled BROWSER session is redirected to /_layouts/15/throttle.htm,
#   which arrives as 406 because the script asked for JSON; detect it on the
#   final URL, not the status, and hold every lane on one gate
#   (_http.js.j2:36-44, 45-63).
# - error.message.value is the server's reason; a bare status left a blocked
#   run undiagnosable (_http.js.j2:25-34, live finding 2026-07-24).
# - the contextinfo parse is guarded step by step; the blind
#   .d.GetContextWebInformation.FormDigestValue chain is what reported
#   dbml-sharepoint #282 as a TypeError (_digest_cached.js.j2:9-42).
# - a title inside getbytitle('...') is an OData literal: apostrophes are
#   doubled, then the whole is URI-encoded (_site_guard.js.j2:24-27).
_JS_PRELUDE = """(async () => {
